...
class GeneralMjsoulError(Exception):
...
class MjsoulChannel():
    ...
    async def call(self, methodName, **msgFields):
        ...
        Use this instead of MjsoulChannel.send
        ...
        if resMessage.error.code:
            print(resMessage)
            raise GeneralMjsoulError(resMessage.error.code, 'error')
...
async def main():
    import proto.combined.lq_dhs_pb2 as lq_dhs
    import mjsoul
    import uuid

    channel = MjsoulChannel(lq_dhs, log_messages=False)
